locals {
  integrations_list = [for k, r in var.integrations : k]
}
...
#=======================#
resource "aws_cognito_user_pool" "this_1" {
...
resource "aws_cognito_user_pool_client" "this" {
  ...
  user_pool_id = aws_cognito_user_pool.this_1[0].id
}
...
resource "aws_api_gateway_resource" "this_nested" {
  ...
  parent_id = aws_api_gateway_resource.this[each.key].id
  rest_api_id = aws_api_gateway_rest_api.this.id
}
